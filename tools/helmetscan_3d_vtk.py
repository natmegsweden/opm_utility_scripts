# -*- coding: utf-8 -*-
"""
3-D VTK visualisation of a helmet scan.

Created on Thu Dec 12 10:24:11 2024
@author: Judith Recober

Hard dependencies: vtk, PyQt5.
These are intentionally NOT imported at the package level — they are isolated
here in ``tools/`` so the rest of the package import chain is unaffected.
"""
...
# Default CSV lives alongside this script.
_DEFAULT_CSV = Path(__file__).parent / 'data_sensors.csv'
...
def choose_files():
    ...
    dialog.setFilter(dialog.filter() | QDir.Hidden | QDir.AllDirs)
...
def render_visualization(smoothed_poly_data, senslabel, empty_slots_aprox_positions):
    ...
    mesh_actor.GetProperty().SetColor(1.0, 0.5, 0.0)
    ...
    for key in empty_slots_aprox_positions:
        ...
        sphere_source2.SetCenter(x2, y2, z2)
        sphere_source2.SetRadius(0.005)
...
if __name__ == "__main__":
    helmet_slots_info = pd.read_csv(_DEFAULT_CSV)
    ...
    slots_info = np.vstack((
        input_data["Coil Name"],
        input_data["sensor_x"],
        input_data["sensor_y"],
        input_data["sensor_z"],
    )).T
